...
class TrainInvariant(pl.LightningModule):
    def __init__(self, in_dim, f_embed_dim,  g_embed_dim, out_dim, num_domains, warmup_length, layers=2):
        super().__init__()
        self.in_dim=in_dim
        self.warmup_length=warmup_length
        self.invariant=TemporalConvNet(in_dim,[f_embed_dim for i in range (layers)])
        self.train_variants=nn.ModuleList([TemporalConvNet(in_dim,[g_embed_dim for j in range (layers)])for i in range (num_domains)])
        self.invariant=TemporalConvNet(in_dim,[g_embed_dim for i in range (layers)])
